...
class Pet
  ...
  def all_info
    puts "Ситість: #{@hangry}"
    puts "Не хочу спати: #{@sleep / 6}"
    puts "Щастя: #{@happy}"
    puts "Не хочу гуляти: #{@walk / 5}"
    puts "Стан іграшок: #{@toys / 4}"
  end

  def getinfo
    "Життя: #{@hp} Ситість: #{@hangry} Не хочу спати: #{@sleep} Щастя: #{@happy} Не хочу гуляти: #{@walk} Стан іграшок: #{@toys}"
  end

  def sum_of_stats
    @hangry + @sleep + @happy + @walk + @toys
  end
  ...
  def minus_hp
    info = "Життя: #{@hp} Ситість: #{@hangry} Не хочу спати: #{@sleep} Щастя: #{@happy} Не хочу гуляти: #{@walk} Стан іграшок: #{@toys}"
    sum = @hangry + @sleep + @happy + @walk + @toys
    hp = @hp
    makeHTML(sum, hp, info) unless @hp.positive?
    puts('Ось і втратили ви свого друга, через недбальсво. Сором вам.') unless @hp.positive?
    exit unless @hp.positive?
  end
