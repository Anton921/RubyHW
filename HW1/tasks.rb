...
puts 'Завдання 1'
m1 = push1(M)
m1.each_with_index { |num, i| puts num if i.even? }
m1.each_with_index { |num, i| puts num if i.odd? }
puts '-----------------------------------'
...
def arif(matrix)
  ...
  (1...matrix.length).each do |i|
    k += 1 if (matrix[i] - matrix[i - 1]) == (matrix[1] - matrix[0])
  end
  if k == (matrix.length - 1)
    puts(matrix[1] - matrix[0])
  else
    puts nil
  end
end
...
puts 'Завдання 16'
def geom(matrix)
  k1 = (matrix[0] * (((matrix[1] / matrix[0])**matrix.length) - 1)) / ((matrix[1] / matrix[0]) - 1)
  k2 = 0
  matrix.each { |num| k2 += num }
  if k1 == k2
    puts matrix[1] / matrix[0]
  else
    puts nil
...
q = 0
w = 0
(1...m1.length).each do |i|
  if (m1[i] - m1[i - 1]) == (m1[q + 1] - m1[q]) && (i != (m1.length - 1))
    k += 1
  elsif (m1[i - 1] - m1[i - 2]).positive?
    q = i
    w += 1
...
k = 4
m2.clear
m1.each { |num| m2.push((num - k).abs) }
(m2.index(m2.min)...m2.length).each do |i|
  puts i + 1 if m2[i] == m2.min
end
puts '-----------------------------------'
...
m2.clear
m1.each { |num| m2.push((num - k).abs) }
(m2.index(m2.max)...m2.length).each do |i|
  puts i + 1 if m2[i] == m2.max
end
puts '-----------------------------------'
...
puts 'Завдання 26'
m1 = [1, -1, 2, -2, 3, -3]
m1.map! { |element| element.negative? ? [m1[0], element] : element }.flatten!
puts m1.join(' ')
puts '-----------------------------------'
...
puts 'Завдання 27'
m1 = [1, -1, 2, -2, 3, -3]
m1.map! { |element| element.positive? ? [m1[0], element] : element }.flatten!
puts m1.join(' ')
puts '-----------------------------------'
...
puts 'Завдання 41'
m1 = [5, 20, 3, 11, 1, 17, 3, 12, 8, 10]
m1.reject! { |num| num if num.odd? }
m1.each { |num| puts num if num == m1.min }
puts '-----------------------------------'

# 42 Дан целочисленный массив. Найти минимальный нечетный элемент.
puts 'Завдання 42'
m1 = [5, 20, 3, 11, 1, 17, 3, 12, 8, 10]
m1.reject! { |num| num if num.even? }
m1.each { |num| puts num if num == m1.min }
puts '-----------------------------------'
...
m1 = [5, 20, 3, 11, 1, 17, 3, 12, 8, 10]
m1.reject! { |num| num if num.odd? }
m1.each { |num| puts num if num == m1.max }
puts '-----------------------------------'
