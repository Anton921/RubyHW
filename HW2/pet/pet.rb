...
module Pet # Зачем здесь нужен модуль?

  class Pet
    # Створюю змінні які відповідають за звикання до певного типу їжі або іграшки

    def initialize(name) # ініціалізую змінні
      @name = name
      @hangry = 10
      @sleep = 60
      @happy = 10
    ...
    def do # Не называй так метод, 'do' используется для того, чтобы обозначить начало блока.
      pass_time(1, 1, 1, 1, 1)
      check_all
    end
    ...
    def all_info
      ...
      puts 'Не хочу спати: ' + (@sleep / 60).to_s
      puts 'Щастя: ' + @happy.to_s
      puts 'Не хочу гуляти: ' + (@walk / 50).to_s
      puts 'Стан іграшок: ' + (@toys/40).to_s
    end
